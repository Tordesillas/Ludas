...
export function getMovableTokens(color: string, stepsToDo: number): Token[] {
    const playerTokens = store.getState().game.tokens.filter((token) => token.color === color);

    const nextTiles = playerTokens.map((token) => tileInNStep(token.tileId, stepsToDo, color));

    return playerTokens.filter((_, index) => !isTileOutOfBound(nextTiles[index]));
}
...
export function findBestTokenToMove(color: string, stepsToDo: number): Token | null {
    ...
    if (availableTokens.length === 0) {
        ...
        const weightedTokenMoves = availableTokens.map((token) => ({
            token,
            weight: evaluateTokenMove(token, stepsToDo)
        }));
...
function evaluateTokenMove(token: Token, dice: number): number {
    ...
    const hasBestProgress = !store
        .getState()
        .game.tokens.filter(({ color }) => color === token.color)
        .some(
            ({ tileId, color }) =>
                getTokenProgress(tileId, color) > getTokenProgress(token.tileId, color) && tileId < 100
        );
    ...
    const howManyTokensBehind = store
        .getState()
        .game.tokens.filter(({ color }) => color !== token.color)
        .filter(({ tileId }) => (token.tileId - 6 + 52) % 52 <= tileId && tileId <= token.tileId).length;
    const dangerBonus = howManyTokensBehind * 3;

    const bonus =
        canKillBonus + farAwayBonus + bestProgressBonus + secureBonus + secureMalus + homeColumnBonus + dangerBonus;
